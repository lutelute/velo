...
2. **Service layer** (`src/services/`): All business logic. Plain async functions (not classes, except `GmailClient`).
   ...
   - `jmap/` — JMAP (RFC 8620/8621) email provider. `client.ts` manages session discovery and authenticated JSON API calls via Tauri HTTP plugin fetch. `jmapSync.ts` handles initial sync (batch fetch) and delta sync via `Email/changes` with server state tokens. `mailboxMapper.ts` maps JMAP mailbox roles to Gmail-style labels. `autoDiscovery.ts` tries `.well-known/jmap` and known providers (Fastmail). `clientFactory.ts` creates JmapClient instances from account records. `types.ts` defines JMAP protocol types.
...
- **JMAP**: JSON-over-HTTP protocol (RFC 8620/8621). TypeScript implementation (no Rust needed). Uses Tauri HTTP plugin `fetch` to bypass CSP. Session discovery via `.well-known/jmap`. Auth: Basic (app passwords) or Bearer tokens. Delta sync via `Email/changes` with server state tokens. Server provides `threadId` — no client-side JWZ threading needed
- **JMAP auto-discovery**: Known providers: Fastmail (`api.fastmail.com/jmap/session`). Falls back to `https://{domain}/.well-known/jmap` (RFC 8620 Section 2.2), then manual URL entry
- **Provider abstraction**: All sync/send operations go through `EmailProvider` interface — use `getEmailProvider(account)` from `providerFactory.ts`, never call Gmail, IMAP, or JMAP APIs directly from components
